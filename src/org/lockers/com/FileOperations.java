...
String dir = System.getProperty("user.dir").concat("\\userFiles\\");
 ...
 int fileSearch()
{
	...
	while (min <= max) {
		mid = (min + max) / 2;
		if (fileNames[mid].compareTo(key) < 0) { min = mid + 1; } else  if(fileNames[mid].compareTo(key) > 0) {
			max = mid - 1;
		}
		else
		{ return 1;}
       ...
       void businessOperations()
	{
		...
		case 4:ui.userInterface(); break;
		
		case 5:ui.exit(); break;
		}
	}
